// Repair TOW (REPAIR > Repair TOW) — the kinds of work that come in.
//
...
// Route: /app/repair-tow
frappe.pages["repair-tow"].on_page_load = function (wrapper) {
	const page = frappe.ui.make_app_page({ parent: wrapper, title: __("Repair TOW"), single_column: true });
	...
	$(page.main).append(`
		<style>
		#page-repair-tow .container{max-width:100%;}
		...
		<div class="rm-wrap">
			<div class="rm-bar">
				<input class="rm-new" placeholder="${__("a new type of work")}">
			...
			<div class="rm-box"><table class="rm-t"><thead><tr>
				<th>${__("Type of Work")}</th><th style="width:120px;">${__("In use")}</th>
	...
	function paint() {
		...
		const rows = S.rows.filter((r) => (S.showRetired || r.active) &&
			(!q || (r.work_name || "").toLowerCase().includes(q)));
		...
		root.find(".rm-body").html(rows.length ? rows.map((r) => `
			<tr data-n="${esc(r.name)}" class="${r.active ? "" : "off"}">
				<td><span class="rm-name">${esc(r.work_name)}</span>
	...
	function load() {
		frappe.call({ method: API + ".get_repair_work_types", args: { include_inactive: 1 }, freeze: false })
			.then((r) => {
				S.rows = r.message || [];
				// how many repairs already name each one — what makes deleting safe or not
				return frappe.call({ method: API + ".repair_work_type_usage", freeze: false });
			})
			.then((r) => {
	...
	function add() {
		...
		if (!v) return;
		frappe.call({ method: API + ".add_repair_work_type", args: { work_name: v } }).then(() => {
	...
	root.on("click", ".ren", function () {
		...
		frappe.prompt({ fieldname: "v", fieldtype: "Data", label: __("New name"), default: n, reqd: 1 },
			(v) => frappe.call({ method: API + ".set_repair_work_type", args: { name: n, work_name: v.v } })
	...
	root.on("click", ".act", function () {
		...
		frappe.call({ method: API + ".set_repair_work_type", args: { name: n, active: row && row.active ? 0 : 1 } })
			.then(load);
	});
	root.on("click", ".del", function () {
		const n = $(this).closest("tr").data("n");
		frappe.confirm(__("Delete <b>{0}</b>?", [esc(n)]),
			() => frappe.call({ method: API + ".delete_repair_work_type", args: { name: n } }).then(load));
	});
	...
	frappe.pages["repair-tow"].on_page_show = load;
};
