...
//
// Product Transfer Records (Delivery > Delivery Records) — every movement a finished piece makes: a change of holder, a re-filing into another bucket, or a return to the floor for rework. Three tables, one timeline.
//
// One line per move, newest first: the piece, where it went from and to, when,
// and who did it. Read only.
// Route: /app/product-transfer-records

frappe.pages["product-transfer-records"].on_page_load = function (wrapper) {
	const page = frappe.ui.make_app_page({ parent: wrapper, title: __("Product Transfer Records"), single_column: true });
	...
	root.append(`
		<style>
		#page-product-transfer-records .container{max-width:100%;}
	...
	// holder, bucket and rework are three different things that happen to a
	// finished piece; the kind is the first thing anyone filters by
	function paintChips() {
		root.find(".dr-chips").html(["", "Holder", "Bucket", "Rework"].map((k) => `
			<button class="dr-c ${KIND === k ? "on" : ""}" data-k="${esc(k)}">${
				k ? __(k) : __("All")}</button>`).join(""));
	}
	root.on("click", ".dr-c", function () { KIND = $(this).data("k") || ""; paintChips(); load(); });


	function load() {
		return frappe.call({ method: API + ".get_product_transfer_records", freeze: false, args: {
	...
	function paint() {
		...
		root.find(".dr-kpis").html(`
			...
			<div class="dr-kpi"><div class="k">${__("By kind")}</div>
				<div class="v" style="font-size:15px;">${Object.entries((t.by_kind || {}))
					.map(([k, n]) => `${esc(k)} ${n}`).join(" · ") || "—"}</div>
				<div class="sub">${__("holder · bucket · rework")}</div></div>`);
		root.find(".dr-t thead").html(`<tr>
			<th>${__("Kind")}</th><th>${__("Piece")}</th><th>${__("Design")}</th>
		...
		root.find(".dr-t tbody").html((DATA.rows || []).length ? DATA.rows.map((r) => `
			<tr>
				<td><span class="dr-tag ${esc((r.kind || "").toLowerCase())}">${esc(r.kind)}</span></td>
				...
				<td class="dr-when">${esc(r.note || "")}</td>
			</tr>`).join("")
	...
	frappe.pages["product-transfer-records"].on_page_show = () => { load(); };
};
