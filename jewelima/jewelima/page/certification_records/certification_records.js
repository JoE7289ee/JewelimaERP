...
//
// Certification Records (Delivery > Delivery Records) — what happened, after it happened.
//
// The certification desks are built around work still to do. This is the other side:
...
// Route: /app/certification-records

frappe.pages["certification-records"].on_page_load = function (wrapper) {
	const page = frappe.ui.make_app_page({ parent: wrapper, title: __("Certification Records"), single_column: true });
	...
	root.append(`
		<style>
		#page-certification-records .container{max-width:100%;}
	...
	F.extra = mk(".f-extra", { fieldtype: "Select", label: __("Certification"), fieldname: "extra",
		options: [""], change: () => load() });
	...
	function paint() {
		...
		root.find(".dr-kpis").html(`
			...
			<div class="dr-kpi"><div class="k">${__("Came back")}</div><div class="v">${t.back || 0}</div>
				<div class="sub">${__("piece(s)")}</div></div>`);
		root.find(".dr-t thead").html(`<tr>
			<th>${__("Batch")}</th><th>${__("Certification")}</th><th>${__("Status")}</th>
	...
	frappe.pages["certification-records"].on_page_show = () => { load(); };
};
