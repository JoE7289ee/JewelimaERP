...
frappe.pages["item-stock"].on_page_load = function (wrapper) {
	...
	$(page.main).append(`
		<style>
		.is-bar{display:grid;grid-template-columns:1.4fr;gap:6px;max-width:420px;margin:2px 0 12px;}
		...
		<div class="is-bar"><div class="is-wh"></div></div>
	...
	function renderBody() {
		$body.html(
			sortedRows()
				...
				.join("") || `<tr><td colspan="4" style="text-align:center;color:var(--text-muted);padding:18px;">No stock here.</td></tr>`
		);
		const gold = rows.filter((r) => r.uom !== "Carat").reduce((s, r) => s + flt(r.qty), 0);
		const ct = rows.filter((r) => r.uom === "Carat").reduce((s, r) => s + flt(r.qty), 0);
		$foot.html(`<tr><td colspan="2">Totals (${rows.length} items)</td><td class="num">${gold.toFixed(3)} g${ct ? " · " + ct.toFixed(3) + " ct" : ""}</td><td></td></tr>`);
	}
