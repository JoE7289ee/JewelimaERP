...
frappe.pages["bag-split"].on_page_load = function (wrapper) {
	...
	function renderDetails(showStart) {
		...
		// every row shows ITS OWN weight. A gold row used to show the bag's whole
		// gold figure, so a card holding two metals printed the full weight twice
		// and read as if it held double what it does.
		const totWt = (it) => flt(it.total != null ? it.total : it.per_piece.reduce((s, p) => s + flt(p.weight), 0));
		const rows = d.items
			.map((it) => `<tr>
		...
		$actions.empty();
		if (showStart) {
			$(`<button class="btn btn-primary btn-sm">${__("EXTRACT")}</button>`).appendTo($card).wrap('<div style="margin-top:12px;"></div>');
