...
//
// Transfer Bucket Records (Delivery > Delivery Records) — which shelf a piece stopped being on. A bucket is where a thing is KEPT, not a stock location, so nothing about the piece's stock or weights changed with it — but who moved it, and when, is still a question the desk gets asked.
//
// One line per move, newest first: the piece, where it went from and to, when,
// and who did it. Read only.
// Route: /app/bucket-transfer-records

frappe.pages["bucket-transfer-records"].on_page_load = function (wrapper) {
	const page = frappe.ui.make_app_page({ parent: wrapper, title: __("Transfer Bucket Records"), single_column: true });
	...
	root.append(`
		<style>
		#page-bucket-transfer-records .container{max-width:100%;}
	...
	function load() {
		return frappe.call({ method: API + ".get_bucket_transfer_records", freeze: false, args: {
	...
	function paint() {
		...
		root.find(".dr-t tbody").html((DATA.rows || []).length ? DATA.rows.map((r) => `
			<tr>
				...
				<td>${esc(r.from_bucket || "—")}</td>
				<td><b>${esc(r.to_bucket || "—")}</b></td>
				...
				<td class="dr-when">${esc(r.remarks || "")}</td>
			</tr>`).join("")
	...
	frappe.pages["bucket-transfer-records"].on_page_show = () => { load(); };
};
