...
//
// Setup > Masters > Purchase Masters — Voucher Types with code + full name + usage; add
// inline; click a row to drill into what uses it. Route: /app/purchase-masters

frappe.pages["purchase-masters"].on_page_load = function (wrapper) {
	const page = frappe.ui.make_app_page({ parent: wrapper, title: "Purchase Masters", single_column: true });
	...
	$(page.main).append(`
		...
		<div class="pm-grid"><div class="pm-card">
			<div class="h"><span class="t">${__("Voucher Types")}</span><span class="s">${__("code up to 4 — leads the purchase series (SIN-0001); more purchase masters land here later")}</span></div>
	...
	function load() {
		frappe.call({ method: API + ".get_party_masters" }).then((r) => {
			rows = ((r.message || {})["voucher"]) || [];
			root.find(".pm-body").html(rows.map((x) => `
				<tr data-code="${esc(x.code)}"><td class="c">${esc(x.code)}</td>
				<td>${esc(x.label || "")}</td>
				<td class="u">${x.customers ? __("{0} purchase(s)", [x.customers]) : __("unused")}</td></tr>`).join("")
	...
	function add() {
		...
		frappe.call({ method: API + ".add_party_master", args: { kind: "voucher", code, label } }).then(() => {
	...
	root.on("click", "table.pm-tbl tr[data-code]", function () {
		...
		frappe.call({ method: "frappe.client.get_list", args: { doctype: "Purchase Record",
			...
			root.find(".pm-cust-t").text(__("{0} — {1} purchase(s)", [code, list.length]));
			root.find(".cb").html(list.length ? `<table><tbody>${list.map((p) => `
