...
def get_selection_photos(search=None, design_type=None, provider=None, tag=None,
	...
	if cts_min not in (None, ""):
		rows = [r for r in rows if flt(r.dmd_weight) >= flt(cts_min)]
	if cts_max not in (None, ""):
		rows = [r for r in rows if flt(r.dmd_weight) and flt(r.dmd_weight) <= flt(cts_max)]
...
def get_selection_records(party=None, from_date=None, to_date=None, limit=300):
	...
	rows = frappe.get_all("Selection", filters=filters,
		fields=["name", "party", "selection_date", "batch", "total_photos",
			...
			"total_dmd_no", "total_dmd_weight", "total_cs_no", "total_cs_weight", "creation"],
	...
	out = [{
		...
		"total_dmd_no": cint(r.total_dmd_no), "total_dmd_weight": flt(r.total_dmd_weight),
		"total_cs_no": cint(r.total_cs_no), "total_cs_weight": flt(r.total_cs_weight),
		"total_dmd_no": cint(r.total_dmd_no), "total_dmd_weight": flt(r.total_dmd_weight),
		"total_cs_no": cint(r.total_cs_no), "total_cs_weight": flt(r.total_cs_weight),
		"preview": previews.get(r.name, ""),
	} for r in rows]
...
def get_selection_review(status="pending", search=None, limit=100):
	...
	rows = frappe.get_all("Selection Photo", filters=filters,
		fields=["name", "code", "image", "gold_18k", "gold_14k", "gold_9k",
			"dmd_no", "dmd_weight", "cs_no", "cs_weight", "stock_pcs", "design_type", "provider", "reviewed"],
...
def review_save(name, gold_18k=None, gold_14k=None, gold_9k=None,
		dmd_no=None, dmd_weight=None, cs_no=None, cs_weight=None,
	...
	for fld, val in (("gold_18k", gold_18k), ("gold_14k", gold_14k), ("gold_9k", gold_9k),
			("dmd_weight", dmd_weight if dmd_weight is not None else cts), ("cs_weight", cs_weight)):
		if val is not None:
			setattr(doc, fld, flt(val))
	for fld, val in (("dmd_no", dmd_no), ("cs_no", cs_no)):
		if val is not None:
			setattr(doc, fld, cint(val))
	if cts is not None:
		doc.dmd_weight = flt(cts)
	if stock_pcs is not None:
		doc.stock_pcs = max(cint(stock_pcs), 0)
	if design_type is not None:
		doc.design_type = design_type or None
...
def review_rename_code(name, new_code):
	...
	if frappe.db.exists("Selection Photo", new_code):
		fields = ["name", "code", "image", "gold_18k", "gold_14k", "gold_9k",
			"dmd_no", "dmd_weight", "cs_no", "cs_weight", "stock_pcs",
...
def get_selected_pieces(party=None, batch=None, from_date=None, to_date=None, selection=None):
	...
	sels = frappe.db.sql("""
		SELECT s.name, s.party, s.selection_date, s.batch, s.total_photos,
		       s.total_gold_18k, s.total_gold_14k, s.total_gold_9k,
		       s.total_dmd_no, s.total_dmd_weight, s.total_cs_no, s.total_cs_weight, s.remarks
	...
	return {
		...
		"total_dmd_no": sum(cint(r.dmd_no) for r in items),
		"total_dmd_weight": round(sum(flt(r.dmd_weight) for r in items), 3),
		"total_cs_no": sum(cint(r.cs_no) for r in items),
		"total_cs_weight": round(sum(flt(r.cs_weight) for r in items), 3),
		"total_dmd_weight": round(sum(flt(r.dmd_weight) for r in items), 3),
...
def get_recent_selections(limit=15):
	...
	return frappe.get_all("Selection", fields=["name", "party", "selection_date", "batch",
		"total_photos", "total_gold_18k", "total_gold_14k", "total_gold_9k",
		"total_dmd_no", "total_dmd_weight", "total_cs_no", "total_cs_weight"],
...
def export_old_sale_jos(priced, price_chart, gold_rate, quality, karat_label="18 KT",
	...
	for col in SUMCOLS:
		L = get_column_letter(col)
		parts = ["{0}{1}".format(L, v) if kind == "cell" else "SUM({0}{1}:{0}{2})".format(L, v[0], v[1])
...
def get_confirm_pool():
	...
	stands — pending / confirmed / rejected / stone (away for a stone change) —
	grouped by batch, live-refreshable.

	A batch whose pieces are all processed normally leaves the pool. One holding a
	piece that is still AWAY for a stone change does not: the desk asked for those
	to stay visible, in yellow, until the stones are changed and the piece comes
	back. The bag's own stock_status is what says whether it is still away, so a
	piece released on the Stone Changes desk settles here by itself."""
	...
	batches = []
	for c in frappe.get_all("Certification", filters={"name": ["in", names]},
			fields=["name", "cert_type", "certification_type", "quality", "collected_on", "status"],
			order_by="collected_on asc, creation asc") if names else []:
		items = frappe.get_all("Certification Item", filters={"parent": c.name},
			fields=["name", "order_bag", "design_type", "received", "rejected",
				"stone_change", "confirmed_by"], order_by="idx")
		away = {}
		...
		batches.append({"name": c.name, "cert_type": c.cert_type or c.certification_type or "?",
			...
			"status": c.status, "pieces": pieces})
	pend = sum(1 for b in batches for p in b["pieces"] if p["state"] == "pending")
	stone = sum(1 for b in batches for p in b["pieces"] if p["state"] == "stone")
...
def _confirm_cert_one(nm, mode, touched):
	...
	if r.rejected:
		return {"rejected_scan": frappe._("Already rejected")}
	if r.stone_change:
	...
	field = {"accept": "received", "reject": "rejected", "stone": "stone_change"}[mode]
...
def _confirm_cert_rollup(parents):
	...
	for parent in parents:
		left = frappe.db.sql("""select count(*) from `tabCertification Item`
			where parent = %s and received = 0 and rejected = 0 and stone_change = 0""",
			parent)[0][0]
		# backwards too: undoing a line on a Received batch reopens it
		cur = frappe.db.get_value("Certification", parent, "status")
		if cur in ("Collected", "Partially Received", "Received"):
...
# Stone change
#
# The lab hands a piece back because a stone has to be replaced. It is not
# rejected — a rejected piece simply goes without its stamp and stays sellable —
# and it is not finished either: it owes work before it can go out again.
#
# So it leaves stock. The materials move Finished Goods -> Stone Change, the bag
# reads Stone Change at the STONE CHANGE location, and the piece stays yellow on
# the Confirm desk until the work is done. Closing the batch puts it back In
# Stock in its own bucket, free to be scanned onto a fresh certification batch —
# which is the "goes back out, comes back in" half of the round trip.
...
def _open_stone_change(pieces):
	...
	bags = [p["bag"] for p in pieces]
	rows = []
	for p in pieces:
		nm = p["bag"]
		b = frappe.db.get_value("Order Bag", nm,
			["design", "act_gross_weight", "act_dmd_weight"], as_dict=True) or frappe._dict()
		rows.append({"order_bag": nm, "design": b.design or "",
			"design_type": (frappe.db.get_value("Design", b.design, "design_type") if b.design else "") or "",
			"gross": flt(b.act_gross_weight), "dmd_ct": flt(b.act_dmd_weight),
	...
	for mats in _bag_convert_materials(bags).values():
		for it, q in mats.items():
			totals[it] = totals.get(it, 0) + q
	d.stock_entry = _stock_move_many(totals, _wh("Finished Goods"), _wh(STONE_CHANGE_WAREHOUSE))
	d.save(ignore_permissions=True)
	for nm in bags:
		frappe.db.set_value("Order Bag", nm,
			{"stock_status": "Stone Change", "location": STONE_CHANGE_LOCATION})
	return {"name": d.name, "count": len(rows), "stock_entry": d.stock_entry}
	...
	for c in frappe.get_all("Stone Change",
		...
		items = frappe.get_all("Stone Change Item", filters={"parent": c.name},
			fields=["name", "order_bag", "design", "design_type", "gross", "dmd_ct",
				"from_certification", "note"], order_by="idx")
		row = dict(c)
		row.update({
			"opened_on": str(c.opened_on or ""), "closed_on": str(c.closed_on or ""),
			"owner_label": _user_label(c.owner),
...
@frappe.whitelist()
def set_stone_change_note(name, row, note=None):
	...
	for r in d.items:
		...
		frappe.throw(frappe._("That piece is not on {0}.").format(name))
	d.save(ignore_permissions=True)
	frappe.db.commit()
	return {"ok": 1}
...
def confirm_cert_batch(changes):
	...
	for c in changes or []:
		...
		results.append(res)
	# every piece marked for a stone change in THIS save becomes one batch — a
	# tray of ten out of fifty is one thing to hand over, not ten loose pieces
	sc = _open_stone_change(to_change) if to_change else None
...
def save_barcode_layout(layout):
	...
	keep = ("pt", "qr", "tag", "a", "b", "lines", "face", "weight", "italic")
...
# ---------------------------------------------------------------------------
# Findings (Stock > Findings) — pipes, tickly, screws. A finding is gold that
# has not joined a piece yet: it sits in the GOLD ISSUE warehouse as itself,
# and the moment it is issued it BECOMES karat gold — onto a card, or into a
# location. Nowhere else in the building does a finding exist as a finding.
# Weight is the unit that counts; a piece count rides along when it helps.
# ---------------------------------------------------------------------------
FINDINGS_ROOT = "GOLD FINDINGS"
FINDING_ROLES = ("System Manager", "Stock Manager", "JW Manager", "JW Stock Admin")


def _findings_guard():
	if not set(FINDING_ROLES) & set(frappe.get_roles()):
		frappe.throw(frappe._("Findings are for the stock desk."), frappe.PermissionError)


def _finding_groups():
	"""Every leaf under GOLD FINDINGS (the groups a finding may belong to)."""
	out = []
	for karat in frappe.get_all("Item Group", filters={"parent_item_group": FINDINGS_ROOT}, pluck="name"):
		out += frappe.get_all("Item Group", filters={"parent_item_group": karat, "is_group": 0}, pluck="name")
	return sorted(out)
...
def _finding_gold_item(item, colour=None):
	"""What this finding turns into. The group name carries it: '18 KPG Findings'
	-> 18KPG. A colourless finding ('18K Common Findings') needs the colour
	picked at issue."""
	g = frappe.db.get_value("Item", item, "item_group") or ""
	m = re.match(r"^(\d{2})\s*K([YWP])G Findings$", g)
	if m:
		return "{0}K{1}G".format(m.group(1), m.group(2))
	m = re.match(r"^(\d{2})K Common Findings$", g)
	if m:
		if not colour:
			frappe.throw(frappe._("{0} has no colour of its own — pick the gold it becomes.").format(item))
		c = colour.upper().replace("G", "").replace("K", "")[:1]
		if c not in ("Y", "W", "P"):
			frappe.throw(frappe._("Colour must be Yellow, White or Pink."))
		return "{0}K{1}G".format(m.group(1), c)
	frappe.throw(frappe._("{0} is not a finding.").format(item))


@frappe.whitelist()
def get_findings_setup():
	"""What the Add Findings page needs: the groups a finding can go into, and
	the karats behind them."""
	_findings_guard()
	groups = []
	for g in _finding_groups():
		karat = (re.match(r"^(\d{2})", g) or [None, ""])[1]
		groups.append({"group": g, "karat": karat + "K" if karat else "",
			"common": "Common" in g})
	return {"groups": groups, "uom": "Gram"}


@frappe.whitelist()
def add_finding(item_code, item_name=None, item_group=None):
	"""Add a new finding to the register. Purity follows its karat — a finding is
	gold, so it is stocked and valued like the karat it belongs to."""
	_findings_guard()
	from jewelima.setup import KARAT_GOLDS

	code = (item_code or "").strip().upper()
	if not code:
		frappe.throw(frappe._("Give the finding a code."))
	if frappe.db.exists("Item", code):
		frappe.throw(frappe._("{0} already exists.").format(code))
	if item_group not in _finding_groups():
		frappe.throw(frappe._("Pick which findings group it belongs to."))
	karat = (re.match(r"^(\d{2})", item_group) or [None, ""])[1] + "K"
	doc = frappe.get_doc({
		"doctype": "Item", "item_code": code, "item_name": (item_name or code).strip(),
		"item_group": item_group, "stock_uom": "Gram", "weight_unit": "Gram",
		"is_stock_item": 1, "is_purchase_item": 1, "is_sales_item": 0,
		"include_item_in_manufacturing": 1, "metal_purity": karat,
		"purity_percentage": KARAT_GOLDS.get(karat, 0),
	}).insert(ignore_permissions=True)
	frappe.db.commit()
	return {"item": doc.name, "group": item_group, "karat": karat}


@frappe.whitelist()
def get_findings_stock():
	"""The findings report: what is on the shelf, by group. Findings only ever
	live in Gold Issue, so anything sitting elsewhere is called out."""
	_findings_guard()
	from jewelima.setup import GOLD_ISSUE_WAREHOUSE

	gold_issue = _wh(GOLD_ISSUE_WAREHOUSE)
	groups = _finding_groups()
	items = frappe.get_all("Item", filters={"item_group": ["in", groups or [""]]},
		fields=["name", "item_name", "item_group", "purity_percentage"], order_by="item_group, name")
	rows, elsewhere = [], []
	for it in items:
		bins = frappe.get_all("Bin", filters={"item_code": it.name, "actual_qty": [">", 0]},
			fields=["warehouse", "actual_qty"])
		here = sum(flt(b.actual_qty) for b in bins if b.warehouse == gold_issue)
		other = [(b.warehouse, flt(b.actual_qty)) for b in bins if b.warehouse != gold_issue]
		rows.append({"item": it.name, "name": it.item_name, "group": it.item_group,
			"purity": flt(it.purity_percentage), "weight": round(here, 3),
			"pure": round(here * flt(it.purity_percentage) / 100.0, 3)})
		for wh, q in other:
			elsewhere.append({"item": it.name, "warehouse": wh, "weight": round(q, 3)})
	rows.sort(key=lambda r: (-r["weight"], r["item"]))
	return {"rows": rows, "elsewhere": elsewhere,
		"warehouse": gold_issue,
		"totals": {"weight": round(sum(r["weight"] for r in rows), 3),
			"pure": round(sum(r["pure"] for r in rows), 3),
			"items": len(rows), "stocked": len([r for r in rows if r["weight"] > 0])}}


@frappe.whitelist()
def issue_finding(item, weight, target_type, order_bag=None, location=None,
		pcs=None, colour=None, remarks=None):
	"""Issue a finding — and in the same breath turn it into gold.

	One Repack does the conversion: the finding leaves Gold Issue, the karat gold
	it becomes lands where it is going. To a CARD that is the In Bags pool plus a
	ledger row, so the card's own materials carry the weight. To a LOCATION the
	gold simply lands in that warehouse."""
	_findings_guard()
	from jewelima.setup import GOLD_ISSUE_WAREHOUSE, IN_PRODUCTION_WAREHOUSE

	w = flt(weight)
	if not item or not frappe.db.exists("Item", item):
		frappe.throw(frappe._("Pick the finding."))
	if not _is_finding(item):
		frappe.throw(frappe._("{0} is not a finding.").format(item))
	if w <= 0:
		frappe.throw(frappe._("Enter the weight going out (g)."))
	src = _wh(GOLD_ISSUE_WAREHOUSE)
	have = flt(frappe.db.get_value("Bin", {"item_code": item, "warehouse": src}, "actual_qty"))
	if w > have + 0.0005:
		frappe.throw(frappe._("Only {0} g of {1} in {2} — can't issue {3} g.").format(
			round(have, 3), item, src, w))

	gold = _finding_gold_item(item, colour)
	if not frappe.db.exists("Item", gold):
		frappe.throw(frappe._("The gold item {0} does not exist.").format(gold))

	target_type = (target_type or "").title()
	if target_type == "Card":
		if not order_bag or not frappe.db.exists("Order Bag", order_bag):
			frappe.throw(frappe._("Scan the card the finding goes onto."))
		if frappe.db.get_value("Order Bag", order_bag, "is_finished"):
			frappe.throw(frappe._("{0} is finished — its materials are locked.").format(order_bag))
		tgt = _wh(IN_PRODUCTION_WAREHOUSE)
		where = order_bag
	elif target_type == "Location":
		tgt = location if location and frappe.db.exists("Warehouse", location) else None
		if not tgt:
			frappe.throw(frappe._("Pick the warehouse the gold goes to."))
		where = tgt
	else:
		frappe.throw(frappe._("Issue to a Card or to a Location."))

	se = frappe.get_doc({
		"doctype": "Stock Entry", "stock_entry_type": "Repack", "company": _company(),
		"items": [
			{"item_code": item, "qty": w, "uom": "Gram", "s_warehouse": src,
				"allow_zero_valuation_rate": 1},
			{"item_code": gold, "qty": w, "uom": "Gram", "t_warehouse": tgt,
				"allow_zero_valuation_rate": 1, "is_finished_item": 1},
		],
		"remarks": "Finding issue: {0} g {1} -> {2} ({3})".format(w, item, gold, where),
	})
	se.flags.ignore_permissions = True
	se.insert()
	se.submit()
	...
	if target_type == "Card":
		# the card carries GOLD from here on — the finding is gone
		_bag_ledger(order_bag, gold, "In", w, "Gold Issue", reference=se.name,
			remarks=(remarks or "") or "Finding: {0}".format(item))
		_recompute_bag_from_contents(order_bag)

	rec = frappe.get_doc({
		"doctype": "Finding Issue", "finding_item": item,
		"finding_name": frappe.db.get_value("Item", item, "item_name"),
		"pcs": cint(pcs) or 0, "weight": w, "target_type": target_type,
		"order_bag": order_bag if target_type == "Card" else None,
		"location": tgt if target_type == "Location" else None,
		"gold_item": gold, "stock_entry": se.name, "remarks": remarks,
	}).insert(ignore_permissions=True)
	frappe.db.commit()
	return {"issue": rec.name, "stock_entry": se.name, "gold_item": gold,
		"weight": w, "to": where}
...
def get_finding_issues(period="month", item=None, limit=300):
	...
	_findings_guard()
	frm, to, label = _loss_period_range(period)
	filters = {}
	if frm and to:
		filters["creation"] = ["between", [frm + " 00:00:00", to + " 23:59:59"]]
	if item:
