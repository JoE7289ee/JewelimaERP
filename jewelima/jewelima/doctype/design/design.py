...
STONE_BUCKET = {"Diamond": "dmd_no", "Precious Stone": "ps_no", "Color Stone": "cs_no", "CVD": "cvd_no", "Party Diamond": "pdmd_no", "Party Other": "poth_no"}

# Fields locked after creation (a design is immutable — you can only retire it).
PROTECTED = ["design_name", "design_type", "design_style", "image", "purity", "dmd_no", "ps_no", "cs_no", "cvd_no", "pdmd_no", "poth_no"]
...
class Design(Document):
	...
	def compute_stone_counts(self):
		...
		counts = {"dmd_no": 0, "ps_no": 0, "cs_no": 0, "cvd_no": 0, "pdmd_no": 0, "poth_no": 0}
		codes = list({m.item for m in self.materials if m.item})
		stype = {}
		if codes:
			for it in frappe.get_all("Item", filters={"name": ["in", codes]}, fields=["name", "stone_type"]):
